...
export default function App() {
  const dbAddress = "http://3.1.24.7:8080/books";
  ...
  const fetchBooks = async () => {
    try {
      ...
      if (data.length > 0) {
        ...
        try {
          const recRes = await fetch("http://3.1.24.7:8080/stars/recommend");
  ...
  const handleUpdateAccount = async () => {
    try {
      const res = await fetch(`http://3.1.24.7:8080/users/${currentUser.userId}`, {
        method: "PATCH",
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const res = await fetch(`http://3.1.24.7:8080/users/${currentUser.userId}`, { method: "DELETE" });
